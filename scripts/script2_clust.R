...
# PURPOSE: To run CyTOF workflow (Robinson's) version 3. This script carries out clustering
...
loginfo("Loading daFrame with arcsinh transformed data saved in script1.")
daf <- readRDS(file.path(results_folder, "daf_arcsinh.rds"))

# CLUSTERING (FlowSom).
loginfo("CLUSTERING (FlowSOM)")
daf <- cluster(daf, cols_to_use = type_markers(daf), xdim = 10, ydim = 10, maxK = no_of_clusters, seed = 1234) 

# Save daFrame 
loginfo("Saving daFrame with clustering data.")
saveRDS(daf, file.path(results_folder, "daf_clust.rds"))
